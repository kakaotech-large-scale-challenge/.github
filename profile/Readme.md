...
- 백엔드
  ...
  - 총 100개의 서비스에 로드밸런싱
  - 파일 서비스와 같은 경우는 S3 버킷에 업로드하고 메타파일을 레디스에 저장하는 방식
- DB
  - 몽고DB로 구축되어 있던 서비스를 Redis로 변경
  - Redis 클러스터링을 통한 정합성 보장
...
- 최대한 부가기능들은 인스턴스를 띄우지 않고 AWS의 서비스로 대체
  ...
  - 마찬가지로 로드밸런싱도 k8s,Nginx 등을 사용하지 않고 ELB 사용
- AutoScailig Group
  - 인스턴스 탬플릿 제작 후 등록
...
- 대용량 트래픽 처리를 해야 하기에 메세지 큐를 사용할까 논의
  - 카프카 vs 레비덴큐 고려시 소켓 통신에 레비덴큐 유리
  - 다만 메시지큐 시스템을 활용했을 때, t3.small 이라는 성능 한계에 의해서 병목지점이 될까 우려
  - 최종적으로 메시지큐를 활용하지않고 Redis 클러스터링을 통한 샤딩으로 대체
- Reids 클러스터링
  - 여기에 밀로가 채우기로함
- 파일 시스템
